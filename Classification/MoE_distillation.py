...
from ClassificationePair.arguments import get_args
from ClassificationePair.MoE_distiller import Distiller
from ClassificationePair.data_utils.distill_datasets import DistillDataset
from ClassificationePair.utils import (
    initialize,
    get_optimizer, 
...
from ClassificationePair.criterions import build_criterion
...
def evaluate(args, tokenizer, student_model, dataset, split, device):
    ...
    for input_batch, output_batch in tqdm(dataloader, desc="Processing batches"):
        ...
        # Check if the model is MoEDistilledBERT or regular model
        if hasattr(student_model, 'moe_layer'):
            # This is MoEDistilledBERT - only pass supported parameters
