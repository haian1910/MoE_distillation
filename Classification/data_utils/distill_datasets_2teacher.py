...
class DistillDataset(Dataset):
        ...
        if os.path.exists(path):
            ...
            for _, row in tqdm(df.iterrows(), total=len(df), disable=(dist.get_rank() != 0)):
                ...
                if self.teacher_tokenizer_2:
                    teacher_input_ids_2 = self.teacher_tokenizer_2.encode(
                        row['text'],
                        add_special_tokens=True,
                        max_length=self.max_length,
                        truncation=True
                    )
                    tokenized_data["teacher_input_ids_2"] = teacher_input_ids_2
    ...
    def collate(self, samples):
        ...
        if self.teacher_tokenizer_2:
            teacher_pad_token_id_2 = self.teacher_tokenizer_2.pad_token_id
            if teacher_pad_token_id_2 is None:
                teacher_pad_token_id_2 = 0
            model_data.update({
                "teacher_input_ids_2": torch.ones(bs, max_length, dtype=torch.long) * teacher_pad_token_id_2,
                "teacher_attention_mask_2": torch.zeros(bs, max_length),
            })
