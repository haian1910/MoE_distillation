...
class DistillDataset(Dataset):
    def __init__(
        ...
        teacher_tokenizer_2: Optional[AutoTokenizer] = None
    ):
        self.args = args
        self.split = split
        self.student_tokenizer = student_tokenizer
        self.teacher_tokenizer = teacher_tokenizer
        self.teacher_tokenizer_2 = teacher_tokenizer_2
    ...
    def _load_and_process_data(self):
        ...
        if os.path.exists(path):
            ...
            for _, row in tqdm(df.iterrows(), total=len(df), disable=(dist.get_rank() != 0)):
                ...
                if self.teacher_tokenizer_2:
                    teacher_encoding_2 = self.teacher_tokenizer_2(
                        row['premise'],
                        row['hypothesis'],
                        add_special_tokens=True,
                        max_length=self.max_length,
                    ...
                    tokenized_data.update({
                        "teacher_input_ids_2": teacher_encoding_2['input_ids'],
                        "teacher_attention_mask_2": teacher_encoding_2['attention_mask'],
                    })
    ...
    def collate(self, samples):
        ...
        if self.teacher_tokenizer_2:
            teacher_pad_token_id_2 = self.teacher_tokenizer_2.pad_token_id or 0
            model_data.update({
                "teacher_input_ids_2": torch.ones(bs, max_length, dtype=torch.long) * teacher_pad_token_id_2,
                "teacher_attention_mask_2": torch.zeros(bs, max_length, dtype=torch.long),
            })
